...
_SINONIMOS: dict[str, list[str]] = {
    ...
    "nota_final":     ["nota final", "calificacion final", "calificación final", "promedio final", "nota_final"],
...
def _detectar_inicio_datos(df: pd.DataFrame) -> int:
    """
    Busca la fila donde empiezan los datos numéricos.
    Los Excels de UPS suelen tener 2-5 filas de encabezado institucional.
    """
    for i, row in df.iterrows():
        valores = [str(v).strip() for v in row.values if str(v).strip()]
        # La fila de encabezados de columna tiene 3+ celdas con texto no numérico
        textos = [v for v in valores if not v.replace(".", "").replace(",", "").isdigit()]
        if len(textos) >= 3:
            return int(str(i))
    return 0
    ...
    if col_map["asignatura"] and col_map["grupo"]:
        ...
        for (nombre_asig, grupo_val), bloque in grupos:
            asig_match = _buscar_asignacion(
                str(nombre_asig), str(grupo_val), asignaciones_periodo
            )
            if asig_match is None:
                advertencias_globales.append(
                    f"Sin asignación registrada: '{nombre_asig}' grupo '{grupo_val}'"
                )
                continue
            estudiantes, adv = _extraer_estudiantes(bloque, col_map, tipo)
            resultados.append(_construir_resultado(asig_match, grupo_val, estudiantes, columnas_presentes, adv))
    else:
